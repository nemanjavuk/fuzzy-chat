...
io.on('connection', function (socket) {
    ...
    //when client gets back online, give him messages he didn't receive while being offline
    socket.on('back online', function (lastDatetime) {

        if (lastDatetime != null) {
            console.log(usernameSession + ' is back online. Give him the messages he did not receive');
            var usernameSession = getUsernameFromSession(socket.request);
            if (usernameSession != undefined) {
                mongo.connect(MONGO_PATH, function (err, db) {
                    var collection = db.collection(CHAT_COLLECTION);
                    collection.find({
                        "message.datetime": {$gt: lastDatetime},
                        "username": {$ne: usernameSession}
                    }).sort({"message.datetime": -1}).toArray(function (err, messages) {
                        if (err) {
                            return;
                        }
                        for (var i = messages.length - 1; i >= 0; --i) {
